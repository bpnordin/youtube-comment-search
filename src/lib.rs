...
pub mod youtube_api { 
    ...
    impl YoutubeVideo {
        ...
        fn youtube_request_builder(
            &self, url: &str, params: HashMap<&str, &str>,
            ) -> Option<Response> {

            let request_get_comments = match self.client.get(url)
                .query(&params)
                .build() {
                    Ok(req) => req,
                    Err(_) => return None
        ...
        pub fn request_video_comment_thread_list(&self, next_page_token: Option<&str>) 
            -> Option<Response> {
            ...
            self.youtube_request_builder(base_url, params)
            
        }

        pub fn request_comment(&self, comment_id: &str) -> Option<Response> {
        ...
        pub fn request_comment_replies(
            &self, comment_id: &str, next_page_token: Option<&str>
            ) -> Option<Response> {
    ...
    fn parse_raw_response(yt_req_response: Option<Response>) 
        -> Option<Value> {
    ...
    fn get_comment_threads(json_response: &Value) -> 
        Vec<YoutubeComment> {
        ...
        for item in item_iter {
                ...
                let text_original = json_object.and_then(|obj| obj.get("snippet"))
    ...
    pub fn search_comments(video: YoutubeVideo, search_term: &str) {
        ...
        loop {
            ...
            let response = {
                parse_raw_response(video.request_video_comment_thread_list(token_str))
                }.unwrap();
        ...
        dbg!(&comment_thread_map);
        dbg!(&comment_thread_map.len());

        //get more YoutubeComment if they have replies
        //set their parent id
        let comment_parent_iter = comment_thread_map.iter()
        ...
        // the logic to go through each should be the same
        // except when calling parentId it does not return the parent comment
       ...
       /* 
        let new_map: HashMap<String,Vec<String>> = comment_thread_map
            .iter()
            .map(|(comment_id, reply_count)| {
                let parsed_comment = self.parse_raw_response(
                    self.request_comment(comment_id)
                    ).unwrap();
                let comment_text = self.parse_video_comment_list(&parsed_comment);
